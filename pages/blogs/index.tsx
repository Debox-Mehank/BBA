...
const Blogs: React.FC = () => {
  const [blogs, setBlogs] = useState<
    BlogsConnectionResponse["blogsConnection"]["edges"]
  >([]);
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const response =
          await graphQLClient.request<BlogsConnectionResponse>(gql`
            query MyQuery {
  ...
  const BlogItem: React.FC<{
    ...
    description: string;
    blurHash: string;
  }> = ({ imageSrc, title, href, description, blurHash }) => {
    return (
      <Link href={href}>
          ...
          <div className="py-4 px-1">
            ...
            <p className="flex items-center font-bebas xsm:text-xl text-lg text-bg1  mt-1">
              ...
              <Image
                src={arrowup}
                alt="arrow-up"
                className="xsm:w-[32px] w-[30px] xsm:h-[32px] h-[30px]"
              />
            </p>
          </div>
        </div>
  ...
  const firstFeaturedBlog =
    featuredBlogs.length > 0 ? featuredBlogs[0].node : null;
      ...
      <div className="blog-content-section px-4 py-8 bg-bg3 min-h-[60vh]">
        <div className="max-w-7xl mx-auto">
          {loading ? (
            ...
            <>
              {firstFeaturedBlog && (
                <BlogFeaturedSection blog={firstFeaturedBlog} />
              )}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-16">
                {blogs
                  .filter(
                    ({ node }) =>
                      !node.featuredBlog ||
                      node.slug !== firstFeaturedBlog?.slug
                  )
                  .map(
                    ({ node }) =>
                      node && (
